		...
		if (event.getSource() == pursue) {
			if (!isPersue) { // To add opportunity to the user
				AsyncCallback<Boolean> callback = new AsyncCallback<Boolean>() {
					@Override
					public void onFailure(Throwable caught) {
						MainPageWidget.setErrorMessage("Failed to add Mentor");
					}
					...
					public void onSuccess(Boolean result) {
						if (result) {
							MainPageWidget
									.setMessage("Mentor added successfully to Opportunity");
						} else {
							MainPageWidget.setMessage("Failed to add Mentor!");
						}
					}
				};
				if (!result.isTypeParticipant()) {
					service.addMentorToOpportunity(result.getO().getId(),
							callback);
				}
				this.hide();
				...
				AsyncCallback<Boolean> callback = new AsyncCallback<Boolean>() {
					...
					public void onFailure(Throwable caught) {
						MainPageWidget
								.setErrorMessage("Failed to delete the Mentor!");
					}
					...
					public void onSuccess(Boolean result) {
						if (result) {
							MainPageWidget
									.setMessage("Mentor Successfully deleted!");
							reloadWidget();
						} else {
							MainPageWidget
									.setErrorMessage("Failed to delete the Mentor!");
				...
				if (!result.isTypeParticipant()) {
					service.removeMentorForOpportunity(result.getO().getId(),
							callback);
				}
				this.hide();
			}
