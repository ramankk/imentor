package com.bugyal.imentor.server.util;
...
public class MyGeocellManager {
	...
	private static final Logger logger = Logger
			.getLogger(MyGeocellManager.class.toString());
	...
	public static List<String> generateGeoCell(Point point) {
		...
		String geocellMax = GeocellUtils.compute(point,
				MyGeocellManager.MAX_GEOCELL_RESOLUTION);
		for (int i = 1; i < MyGeocellManager.MAX_GEOCELL_RESOLUTION; i++) {
	...
	public static final <T extends LocationCapable> List<T> proximityFetch(
		...
		logger.info("Doing proximity search");
		...
		while (!curGeocells.isEmpty()) {
			...
			Query query = pm.newQuery(entityClass, baseQuery.getBaseQuery()
					+ and + "geocells.contains(geocellsP)");
			...
			logger.log(Level.INFO, "fetch complete for: "
			...
			if (results.size() < maxResults) {
				logger.log(Level.INFO, results.size()
		...
		for (Tuple<T, Double> entry : results.subList(0, Math.min(maxResults,
				results.size()))) {
			logger.info("KRK Distance :: " + entry.getSecond());
